;(function(){
    function slidePager(options){
        ...
        this.init();
        slidePager.list.push(this);
    }

    slidePager.prototype = {
        init : function(){
            var _this = this,
            ...
            _this.cur_page = $(options.page_item).eq(0);
    ...
    window.addEventListener(RESIZE_EV, function(){
        ...
        resize_time = setTimeout(function(){
            for(var i = slidePager.list.length - 1; i >= 0; i--){
                slidePager.list[i].resize();
            }
        }, 500);
    });

    window.LightApp = window.LightApp || {};
    LightApp.slidePager = slidePager;
})();
